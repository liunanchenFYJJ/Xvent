...
function createDistributor(
  ...
  const distributor = (next, error, complete) => {
    ...
    const subscription = observable.subscribe(
      value => {
        util_1.deliver([innerObserver].concat(outerObservers), 'next', value);
      },
      error => {
        util_1.deliver([innerObserver].concat(outerObservers), 'error', error);
      },
      () => {
        util_1.deliver([innerObserver].concat(outerObservers), 'complete');
      }
    );
    // 立即返回一个disposer，用来解绑
    return () => {
 ...
 */
function pipe(source, target, extraParams) {
  return source.addReader(value => {
    if (typeof extraParams !== 'undefined') {
      if (util_1.isFunction(extraParams)) {
